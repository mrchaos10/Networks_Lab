...
int main()
{
	...
	printf("\nHostname From Client : %s ",hostname);
        ...
        strcat(query," cache.txt");
        strcat(query,">>temp.txt");
        printf("%s",query);
        system(query);
        int fde=open("temp.txt",O_CREAT |O_RDWR);
	if(fde==-1)
	{
		printf("File Error temp.txt \n");
		close(sockid);
		return 0;
	}
	char content[300];
	lseek(fde,0,0);
	read(fde,content,strlen(hostname));
        //printf("%s \n %s",hostname,content);
        if(strcmp(content,hostname)==0)
        {
        lseek(fde,0,0);
        read(fde,content,30);
        content[30]='\0';
        sentbytes=sendto(sockid,content,strlen(content),0,(struct sockaddr*)&dummy,size);
	system("rm temp.txt");
        close(sockid);
	return 0;
        }
	...
	ip[k]='\0';
        ...
        int fd1=open("cache.txt",O_RDWR | S_IREAD | S_IWRITE);
	if(fd1==-1)
	{
		printf("File Error");
		close(sockid);
	...
	write(fd1,hostname,strlen(hostname));
        write(fd1," ",1);
        write(fd1,ip1,strlen(ip1));
